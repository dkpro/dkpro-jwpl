 ...
 * 	Oliver Ferschke
 ******************************************************************************/
package de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.consumer.sql.codec;
	...
	implements SQLEncoderInterface
{
	...
	/* (non-Javadoc)
	 * @see de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.consumer.sql.codec.SQLEncoderInterface#binaryTask(de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.data.tasks.Task)
	 */
	@Override
		...
		DecodingException, EncodingException, SQLConsumerException
	{
		...
		for (int i = 0; i < size; i++) {
			...
			tempData = "(null, " + this.lastFullRevID + ","
					...
					+ ",?,'"+diff.getComment()+"',"+(diff.isMinor()?"1":"0")+",'"+diff.getContributorID() + "',"+(diff.getContributorIsRegistered()?"1":"0")+")";
	...
	/* (non-Javadoc)
	 * @see de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.consumer.sql.codec.SQLEncoderInterface#encodeTask(de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.data.tasks.Task)
	 */
	@Override
		...
		DecodingException, EncodingException, SQLConsumerException
	{
		...
		for (int i = 0; i < size; i++) {
			...
			tempData = "(null," + this.lastFullRevID + ","
					+ diff.getRevisionCounter() + "," + diff.getRevisionID()
					+ "," + articleId + "," + diff.getTimeStamp().getTime()
					+ ",'" + encodeDiff(task, diff) + "','"+diff.getComment()+"',"+(diff.isMinor()?"1":"0")+",'"+diff.getContributorID() + "',"+(diff.getContributorIsRegistered()?"1":"0")+")";
	...
	/* (non-Javadoc)
	 * @see de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.consumer.sql.codec.SQLEncoderInterface#getBinaryTable()
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see de.tudarmstadt.ukp.wikipedia.revisionmachine.difftool.consumer.sql.codec.SQLEncoderInterface#getTable()
	 */
	@Override
