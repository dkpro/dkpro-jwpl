...
/**
 * Checks the Range of the SrcSpans of a ParsedPage, so it isn't possible that e.g. a ContentElement
 * conatins a Link which isn't in the Range of this ContentElement. This must be done because some
 * positons will be jammed by the parsing process, e.g. if a Link is the start of a Paragrah.
 */
public class SrcPosRangeChecker
{
    ...
    {
        List<SrcSpan> eil = new ArrayList<>();
        for (Span s : ce.getFormatSpans(FormatType.BOLD))
            eil.add(s.getSrcSpan());
        for (Span s : ce.getFormatSpans(FormatType.ITALIC))
            eil.add(s.getSrcSpan());
        for (Span s : ce.getFormatSpans(FormatType.MATH))
            eil.add(s.getSrcSpan());
        for (Span s : ce.getFormatSpans(FormatType.TAG))
            eil.add(s.getSrcSpan());
        for (Span s : ce.getFormatSpans(FormatType.NOWIKI))
            eil.add(s.getSrcSpan());
