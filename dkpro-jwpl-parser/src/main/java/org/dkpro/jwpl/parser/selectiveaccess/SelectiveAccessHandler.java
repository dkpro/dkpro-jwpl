 ...
 */
package org.dkpro.jwpl.parser.selectiveaccess;
...
public class SelectiveAccessHandler
{
    ...
    enum SIT
    {
        SUBS, TITLE, TABLE, DEFLIST, NESTLIST, PARA
    }
    ...
    public static EnumMap<CIT, Boolean> buildCITMap(boolean text, boolean bold, boolean italic,
            boolean link)
    {
    ...
    public static EnumMap<SIT, EnumMap<CIT, Boolean>> buildSITMap(EnumMap<CIT, Boolean> subs,
            EnumMap<CIT, Boolean> title, EnumMap<CIT, Boolean> table, EnumMap<CIT, Boolean> deflist,
            EnumMap<CIT, Boolean> nestedlist, EnumMap<CIT, Boolean> paragraph)
    {
    ...
    /**
     * if firstParagraphHandling is null, there will be no special handling for the
     * FirstParagraph...
     */
    ...
    private void handleSection(Section s, StringBuilder sb)
    {
        ...
        if (s.getTitle() != null)
            hp = sectionHandling.get(SectionType.USER_SECTION + s.getTitle().toUpperCase());
        if (hp == null)
            hp = sectionHandling
                    .get(SectionType.SECTION_LEVEL.toString() + (s.getLevel() - levelModifier));
        if (hp == null)
            hp = sectionHandling.get(SectionType.DEFAULT_SECTION.toString());
        if (hp == null) {
            System.err.println("Cannot get Handling Parameters for Section:\"" + s.getTitle()
                    + "\" Level:" + s.getLevel());
            return;
        }
    ...
    private void loadConfig()
    {
        ...
        setDefaultSectionHandling(
                buildSITMap(buildCITMap(false, false, false, false), null, null, null, null, null));
    }
