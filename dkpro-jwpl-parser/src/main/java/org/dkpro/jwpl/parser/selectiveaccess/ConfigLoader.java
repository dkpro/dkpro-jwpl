    ...
    extends DefaultHandler
{
    ...
    public void startElement(String uri, String localName, String qName, Attributes att)
    {
        if (localName.equalsIgnoreCase("cit")) {
            citm = SelectiveAccessHandler.buildCITMap("true".equalsIgnoreCase(att.getValue("text")),
        ...
        if (localName.equalsIgnoreCase("cit")) {
            ...
            if (name != null)
                if (name.startsWith(SelectiveAccessHandler.SectionType.DEFAULT_SECTION.toString())
                        || name.startsWith(
                                SelectiveAccessHandler.SectionType.SECTION_LEVEL.toString())
                        || name.startsWith(
                                SelectiveAccessHandler.SectionType.USER_SECTION.toString()))
                    sectionHandling.put(name, sitm);
                else
                    sectionHandling.put(SelectiveAccessHandler.SectionType.USER_SECTION + name,
                            sitm);
            else
