...
/**
 * A Class which manages Spans which are related to a StringBuilder. With the SpanManager it is
 * possible to work on a String (delete, insert, replace) with no need to adjust the Spans related
 * to the StringBuilder manually.
 */
    ...
    implements CharSequence
{
    ...
    /**
     * Enables the Calculation of Src Position. The base for these position will be the aktual, not
     * the initial, String wich is uses as Base for the SpanManager.
     */
     ...
     *
     * @return the Position the index has, when enableSrcPosCaulation() has been called, or -1 if it
     *         is not possible.
     */
    ...
    {
        return delete(s.getStart(), s.getEnd());
    }
    ...
    {
        return replace(s.getStart(), s.getEnd(), str);
    }
