...
import org.dkpro.jwpl.parser.NestedListElement;
    ...
    implements MediaWikiParser, MediaWikiContentElementParser
{
        ...
        while (!lineSpans.isEmpty()) {
            ...
            case SECTION:
                ...
                sc = new SectionContent(
                        parseContentElement(sm, cepp,
                                new Span(s.getStart() + level, s.getEnd() - level).trim(sm)),
                        level);
                lineSpans.removeFirst();
    ...
    {
        SECTION, TABLE, NESTEDLIST, NESTEDLIST_NR, DEFINITIONLIST, HR, PARAGRAPH,
        PARAGRAPH_INDENTED, PARAGRAPH_BOXED, EMPTYLINE
    }
    ...
    /**
     * Takes a list of SectionContent and returns a SectionContainer with the given SectionContent s
    ...
    private SectionContainer buildSectionStructure(List<SectionContent> scl)
    {
        ...
        for (SectionContent sContent : scl) {
            ...
            for (int containerLevel = result.getLevel()
                    + 1; containerLevel < contentLevel; containerLevel++) {
                ...
                if (containerSubSections != 0) {
                    ...
                    if (temp.getClass() == SectionContainer.class) {
                        ...
                        SectionContainer sct = new SectionContainer(temp.getTitleElement(),
                                containerLevel);
                        sct.addSection(temp);
                        if (calculateSrcSpans) {
            ...
            String specifier, String prefix)
    {
        ...
        while ((s = getTag(sm, offset)) != null) {
            ...
            if (startsWithIgnoreCase(tagText, specifier)) {
                ...
                if (calculateSrcSpans) {
                    tSpan.setSrcSpan(
                            new SrcSpan(sm.getSrcPos(s.getStart()), sm.getSrcPos(e.getEnd())));
                }
            ...
            List<ResolvedTemplate> resolvedTemplates, ParsedPage pp)
    {
        ...
        while (!templateOpenTags.empty()) {
            ...
            if (calculateSrcSpans) {
                t.setSrcSpan(new SrcSpan(sm.getSrcPos(templateOpenTag),
            ...
            if (startsWithIgnoreCase(openText, "GALLERY")) {
                ...
                if (startsWithIgnoreCase(getTagText(sm, tagSpans.get(i + 1)), "/GALLERY")) {
                    ...
                    if (eqPos != -1) {
                        ...
                        if (captionStart < captionEnd && openText.charAt(captionStart) == '"'
        ...
        while (!lineSpans.isEmpty()) {
            ...
            if (subTables == 0 && (c0 == '!' || c0 == '|')) {
                if (!tableDataSpans.isEmpty()) {
                    ...
                    SrcSpan ei = null;
                    if (calculateSrcSpans) {
                        ei = new SrcSpan(sm.getSrcPos(tableDataSpans.getFirst().getStart() - 1) + 1,
                                -1);
                    }

                    TableElement te = new TableElement(parseSections(sm, cepp, tableDataSpans), row,
                            col);
                    te.setSrcSpan(ei);
                ...
                if (c1 == '-') {
                    ...
                    result.setTitleElement(parseContentElement(sm, cepp,
                            new Span(s.getStart() + pos + 2, s.getEnd()).trim(sm)));
                    continue;
                }
                else {
                    int multipleCols;
                    if ((multipleCols = sm.indexOf("||", s.getStart() + pos + 1,
                            s.getEnd())) != -1) {
    ...
    private NestedListContainer buildNestedList(SpanManager sm,
            ContentElementParsingParameters cepp, LinkedList<Span> lineSpans, lineType listType)
    {
            ...
            LinkedList<Span> lineSpans)
    {
        ...
        if (calculateSrcSpans) {
            result.setSrcSpan(new SrcSpan(sm.getSrcPos(s.getStart()),
                    content.get(content.size() - 1).getSrcSpan().getEnd()));
        }
    ...
    private void parseExternalLinks(SpanManager sm, Span s, String protocol, List<Span> managedList,
            List<Link> links, Content home_cc)
    {
        ...
        while ((extLinkTargetStart = sm.indexOf(protocol, extLinkSpan.getEnd(),
            ...
            Link l = new Link(home_cc, extLinkSpan,
                    sm.substring(extLinkTargetStart, extLinkTargetEnd), Link.type.EXTERNAL, null);
            links.add(l);

            if (calculateSrcSpans) {
                l.setSrcSpan(new SrcSpan(sm.getSrcPos(extLinkOpenTag),
                        sm.getSrcPos(extLinkCloseTag - 1) + 1));
            }
    ...
    /**
     * Building a ContentElement from a single line. But the result is given, so e.g. a
     * NestedListElement can be filled with information...
     */
    private ContentElement parseContentElement(SpanManager sm, ContentElementParsingParameters cepp,
            Span lineSpan, ContentElement result)
    {
        LinkedList<Span> lineSpans = new LinkedList<>();
        lineSpans.add(lineSpan);
    ...
    private static List<Link> sortLinks(List<Link> links)
    {
        ...
        for (Link l : links) {
            ...
            while (pos < result.size()
                    && l.getPos().getStart() > result.get(pos).getPos().getStart()) {
                pos++;
            }
            result.add(pos, l);
        }
    ...
    private static List<Template> sortTemplates(List<Template> templates)
    {
        ...
        for (Template t : templates) {
            int pos = 0;
            while (pos < result.size()
                    && t.getPos().getStart() > result.get(pos).getPos().getStart()) {
                pos++;
            }
            result.add(pos, t);
        }
