 ...
 * <p>
 * This panel contains all components for setting configuration parameters related to the diff
 * calculation.
 */
    ...
    extends AbstractPanel
{
    ...
    private void createFullRevisionSettings()
    {
        ...
        fullRevisionLabel = new JLabel("Every n-th revision will be a full revision:");
    ...
    private void createMinimumCommonSequenceSettings()
    {
        ...
        minimumCommonSequenceLabel = new JLabel("Min lenght of a common subsequence:");
        minimumCommonSequenceLabel.setBorder(BorderFactory.createRaisedBevelBorder());
    ...
    {
        Object o = config.getConfigParameter(ConfigurationKeys.COUNTER_FULL_REVISION);
        if (o != null) {
        ...
        o = config.getConfigParameter(ConfigurationKeys.VALUE_MINIMUM_LONGEST_COMMON_SUBSTRING);
        if (o != null) {
            this.minimumCommonSequenceField.setText(Integer.toString((Integer) o));
        }
        else {
    ...
    public void toXML(StringBuilder builder, final ConfigVerification errors)
    {
            ...
            try {
                ...
                if (minLCS < 7) {
                    errors.add(new ConfigItem(ConfigItemTypes.ERROR,
                            ConfigErrorKeys.VALUE_OUT_OF_RANGE, "The value of the minimum "
                                    + " LongestCommonSubsequence has to be" + " at least 7."));
                }
                else if (minLCS < 12) {
                    errors.add(new ConfigItem(ConfigItemTypes.WARNING,
                            ConfigErrorKeys.VALUE_OUT_OF_RANGE, "A value smaller than 12 for the "
                                    + "minimum LongestCommonSubsequence" + " is not recommended."));
                }
            }
            catch (NumberFormatException nfe) {
                errors.add(new ConfigItem(ConfigItemTypes.ERROR, ConfigErrorKeys.ILLEGAL_INPUT,
                        "NumberFormatException for " + "ArticleProducer TaskLimit"));
            }
        }
            ...
            try {
                ...
                if (fullRevCounter < 1) {
                    errors.add(new ConfigItem(ConfigItemTypes.ERROR,
                            ...
                            "The FullRevision Counter has to " + "be at least 1."));
                }
                else if (fullRevCounter < 100) {
                    errors.add(new ConfigItem(ConfigItemTypes.WARNING,
                            ConfigErrorKeys.VALUE_OUT_OF_RANGE, "A FullRevision Counter with a"
                                    + " value smaller than 100 is not" + " recommended."));
                }
            }
            catch (NumberFormatException nfe) {
                errors.add(new ConfigItem(ConfigItemTypes.ERROR, ConfigErrorKeys.ILLEGAL_INPUT,
        ...
        builder.append(
                "\t\t<COUNTER_FULL_REVISION>" + fullRevCounter + "</COUNTER_FULL_REVISION>\r\n");
        builder.append("\t</values>\r\n");
    }
}
