...
public class RevisionDecoder
{
     ...
     *             if the decoding failed
     */
    private Diff decode(final int blockSize_C, final int blockSize_S, final int blockSize_E,
            final int blockSize_B, final int blockSize_L)
        ...
        throws UnsupportedEncodingException, DecodingException
    {
        ...
        if (blockSize_S < 1 || blockSize_L < 1) {
            throw new DecodingException("Invalid value for blockSize_S: " + blockSize_S
        ...
        throws DecodingException
    {
        ...
        if (blockSize_S < 1 || blockSize_E < 1 || blockSize_B < 1) {
            throw new DecodingException("Invalid value for blockSize_S: " + blockSize_S
                    + ", blockSize_E: " + blockSize_E + " or blockSize_B: " + blockSize_B);
        }
     ...
     *             if the decoding failed
     */
    private DiffPart decodeReplace(final int blockSize_S, final int blockSize_E,
            final int blockSize_L)
        throws UnsupportedEncodingException, DecodingException
    {

        if (blockSize_S < 1 || blockSize_E < 1 || blockSize_L < 1) {
            throw new DecodingException("Invalid value for blockSize_S: " + blockSize_S
                    + ", blockSize_E: " + blockSize_E + " or blockSize_L: " + blockSize_L);
        }
    ...
    public void setInput(final InputStream input, final boolean binary) throws IOException
    {
        ...
        if (!binary) {
            ...
            if (zipFlag) {
                r = new BitReader(inflateInput(decoder.decode(buffer.toString()), 0));
            }
            else {
    ...
    public void setInput(final String input) throws DecodingException
    {
        ...
        Base64.Decoder decoder = Base64.getDecoder();
        if (zipFlag) {
            r = new BitReader(inflateInput(decoder.decode(input.substring(1)), 0));
        }
        else {
