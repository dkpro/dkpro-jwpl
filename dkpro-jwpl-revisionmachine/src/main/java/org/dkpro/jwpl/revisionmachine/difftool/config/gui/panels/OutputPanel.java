 ...
 * <p>
 * This panel contains all components for setting configuration parameters related to the file
 * output.
 */
    ...
    extends AbstractPanel
{
    ...
    private void createOutputSizeSettings()
    {
        ...
        enableMultipleOutputFiles = new JCheckBox("Allow multiple output files per consumer");
    ...
    public void validate()
    {
        ...
        enableZipEncodingCompression.setSelected(controller.isZipCompressionEnabled());

        disableOutputCompression.setSelected(oce == OutputCompressionEnum.None);

        enableBZip2OutputCompression.setSelected(oce == OutputCompressionEnum.BZip2);
        ...
        enable7ZipOutputCompression.setEnabled(flagA && controller.is7ZipEnabled());

        enable7ZipOutputCompression.setSelected(oce == OutputCompressionEnum.SevenZip);
    ...
    public void applyConfig(final ConfigSettings config)
    {
        ...
        o = config.getConfigParameter(ConfigurationKeys.MODE_ZIP_COMPRESSION_ENABLED);
        if (o != null) {
        ...
        o = config.getConfigParameter(ConfigurationKeys.MODE_DATAFILE_OUTPUT);
        if (o != null) {
        ...
        if (o != null) {
            ...
            case UNCOMPRESSED:
                ...
                o = config.getConfigParameter(ConfigurationKeys.LIMIT_SQL_FILE_SIZE);
                break;
            case SEVENZIP:
                controller.setEnableSQLDatabaseOutput(false);
                controller.setOutputCompression(OutputCompressionEnum.SevenZip);

                o = config.getConfigParameter(ConfigurationKeys.LIMIT_SQL_ARCHIVE_SIZE);
                break;
            case BZIP2:
                controller.setEnableSQLDatabaseOutput(false);
                controller.setOutputCompression(OutputCompressionEnum.BZip2);
    ...
    public void toXML(final StringBuilder builder, final ConfigVerification errors)
    {
        ...
        if (!controller.isEnableSQLDatabaseOutput()) {
            ...
            if (multiFile) {
                    ...
                    try {
                        ...
                        if (sizeLimit < 100 * 1024 * 1024) {
                            errors.add(new ConfigItem(ConfigItemTypes.ERROR,
                                    ...
                                    "The output limit has to be at" + " least 100MB"));
                        }
                    }
                    catch (NumberFormatException nfe) {
                        errors.add(
                                new ConfigItem(ConfigItemTypes.ERROR, ConfigErrorKeys.ILLEGAL_INPUT,
                                        "NumberFormatException for the" + " output limit"));
                    }
                }
                ...
                case None:
                    builder.append(
                            "\t\t<LIMIT_SQL_FILE_SIZE>" + sizeLimit + "</LIMIT_SQL_FILE_SIZE>\r\n");
                    break;
                default:
