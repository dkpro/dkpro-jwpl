...
public final class ConsumerLogMessages
{
    ...
    public static void logInitialization(final Logger logger)
    {
        logger.logMessage(Level.INFO,
                "Consumer initialized [LogLevel: " + logger.getLogLevel() + "]");
    }
    ...
    {
        logger.logMessage(Level.INFO, "Consumer initiates SHUTDOWN\t" + Time.toClock(endTime));
    }
     ...
     */
    public static void logStatus(final Logger logger, final long startTime, final long sleepingTime,
            final long workingTime)
    {

        logger.logMessage(Level.DEBUG, "Consumer-Status-Report ["
                + Time.toClock(System.currentTimeMillis() - startTime) + "]" + "\tEFFICIENCY\t "
                + MathUtilities.percentPlus(workingTime, sleepingTime) + "\tWORK  ["
                + Time.toClock(workingTime) + "]" + "\tSLEEP [" + Time.toClock(sleepingTime) + "]");
    }
     ...
     *            reference to the exception
     */
    public static void logTimeoutException(final Logger logger, final TimeoutException e)
    {
