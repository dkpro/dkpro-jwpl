 ...
 * <p>
 * Block C 3bit operation value Block S start position Block E length (end position = start position
 * + length) Block B block id Block L length of the t block Block T block containing L bytes data
 */
public class RevisionCodecData
{
    ...
    /**
     * Converts the input information into their log2 values. If an operation is contained in the
     * diff, the minimum number of bits used to encode this block is 1 byte.
     *
    ...
    public int totalSizeInBits()
    {
        ...
        if (converted) {
            ...
            return 24 + this.countC * 3 + this.countS * blocksize_S + this.countE * blocksize_E
                    + this.countB * blocksize_B + this.countL * blocksize_L + this.countT * 8;
        }
        ...
        if (this.blocksize_B > 0) {
            this.blocksize_B = (int) Math.ceil(Math.log(blocksize_B + 1) / Math.log(2.));
        }
        ...
        if (this.blocksize_E > 0) {
            this.blocksize_E = (int) Math.ceil(Math.log(blocksize_E + 1) / Math.log(2.));
        }
        ...
        if (this.blocksize_L > 0) {
            this.blocksize_L = (int) Math.ceil(Math.log(blocksize_L + 1) / Math.log(2.));
        }
        ...
        if (this.blocksize_S > 0) {
            this.blocksize_S = (int) Math.ceil(Math.log(blocksize_S + 1) / Math.log(2.));
        }
    ...
    /**
     * Returns the number of bits used to encode a B block. This method is intended to be used after
     * the conversion.
     *
    ...
    {
        return this.blocksize_S + " " + this.blocksize_E + " " + this.blocksize_B + " "
                + this.blocksize_L;
    }
