...
public class ChronoFullRevision
{
     ...
     *            last revision counter based on the full revision
     */
    public ChronoFullRevision(final int fullRevisionPK, final int startRC, final int endRC)
    {
    ...
    public Revision getNearest(final int revisionCounter)
    {
        ...
        if (first != null) {
            ...
            ChronoStorageBlock previous = null, current = first;
            while (current != null && current.getRevisionCounter() <= revisionCounter) {
                previous = current;
                current = current.getCounterNext();
            }
    ...
    public long clean(final int currentRevisionIndex, final int revisionIndex)
    {
        ...
        do {
            ...
            if (current.isDelivered()) {
                ...
                if (next != null) {
                    if (current.getRevisionCounter() + 1 == next.getRevisionCounter()) {
                        remove = true;
                    }
                }

            }
            else if (current.getIndexNext() == null && current.getIndexPrev() == null) {
