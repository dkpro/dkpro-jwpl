...
/**
 * Forwards the necessary information to the AbstractIndex classes and controls the writing to the
 * output if one of the index has reached the maximum size.
 */
public class Indexer
{
    ...
    /**
     * Checks whether the AbstractIndex classes have output available and forward them to the output
     * writer.
     *
     * @throws IOException
     *             if an error occurred while writing the output
    ...
    public void index(final Revision rev) throws WikiApiException
    {
        ...
        if (articleID != currentArticleID) {
            ...
            if (lastRev != null) {
                ...
                try {
                    this.articleIndex.add(currentArticleID, startTime, endTime, infoList);
                    send();
                }
            ...
            if (revisionCounter != 1) {
                System.err.println("WARNING : ArticleID (" + articleID
                        + ") RevisionCounter 1 expected - " + revisionCounter + " read");
            }
            ...
            if (lastRev.getRevisionCounter() + 1 != revisionCounter) {
                System.err.println("WARNING : ArticleID (" + articleID + ")" + " RevisionCounter "
                        + (lastRev.getRevisionCounter() + 1) + " expected - " + revisionCounter
                        + " read");
            }
        ...
        chronoIndex.add(articleID, rev.getRevisionCounter(), rev.getTimeStamp().getTime());
        lastRev = rev;
    }
