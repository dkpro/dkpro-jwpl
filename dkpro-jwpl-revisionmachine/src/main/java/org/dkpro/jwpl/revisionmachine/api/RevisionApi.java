    ...
    extends AbstractRevisionService
{
     ...
     *             if an error occurs
     */
    public List<Timestamp> getRevisionTimestampsBetweenTimestamps(int articleID,
     ...
     * <p>
     * It is possible to only count the registered users, if onlyRegistered is set to true <br>
     * In order to make this query fast, create a MySQL-Index (BTREE) on the ArticleID in the
     * revisions-table.
     *
     * @param articleID
     ...
     * the {@code groupFilter}. Nothing is filtered if the {@code groupFilter} is {@code null} or
     * empty.<br>
     * <br>
     ...
     * {@link #getUserContributionMap(int, String[], boolean)} and set
     * {@code onlyRegistered=true}.<br>
     * <br>
     * In order to make this query fast, create a MySQL-Index (BTREE) on the ArticleID in the
     * revisions-table.
     *
        ...
        try {
            ...
            try {
                ...
                if (groupFilter == null || groupFilter.length < 1 || !tableExists("user_groups")) {
                    ...
                    if (!onlyRegistered) {
                        statementStr.append(
                                " UNION ( SELECT ContributorName, Timestamp FROM revisions AS rev WHERE ArticleID=? AND rev.ContributorId IS NULL)");
                    }
        ...
        try {
            ...
            try (PreparedStatement statement = this.connection.prepareStatement(sql)) {
                ...
                if (result.next()) {
                    ...
                    lastPK = firstPK + Integer
                            .parseInt(revisionCounters.substring(index, revisionCounters.length()));

                    if (time.getTime() < firstDate) {
                        throw new WikiPageNotFoundException(
                                "No revision before the specified date [" + time + "]");
                    }
                }
                else {
                    throw new WikiPageNotFoundException(
