...
public class WikipediaXMLWriter {
   ...
   */
  public WikipediaXMLWriter(final String path) throws IOException {
   ...
   * @throws IOException if an error occurs while writing the output
   */
  public void writeDiff(final Task<Diff> diff) throws IOException {
    writeDiff(diff, 0);
  }
   ...
   * @throws IOException if an error occurs while writing the output
   */
  public void writeDiff(final Task<Diff> diff, final int start) throws IOException {
    ...
    for (int i = start; i < size; i++) {
      ...
      this.writer.write("\t\t" + WikipediaXMLKeys.KEY_START_TIMESTAMP.getKeyword());
      this.writer.write(d.getTimeStamp().toString());
      this.writer.write(WikipediaXMLKeys.KEY_END_TIMESTAMP.getKeyword() + "\r\n");

      this.writer.write("\t\t" + WikipediaXMLKeys.KEY_START_TEXT.getKeyword());
   ...
   * @throws IOException if an error occurs while writing the output
   */
  public void writeDiffFile(final Task<Diff> diff) throws IOException {
    ...
    for (int i = 0; i < size; i++) {
      ...
      this.writer.write("\t\t\t<s>" + codec.getBlocksizeS() + "</s>\r\n");
      this.writer.write("\t\t\t<e>" + codec.getBlocksizeE() + "</e>\r\n");
      this.writer.write("\t\t\t<b>" + codec.getBlocksizeB() + "</b>\r\n");
      this.writer.write("\t\t\t<l>" + codec.getBlocksizeL() + "</l>\r\n");
      ...
      for (int j = 0; j < partsCount; j++) {
        ...
        this.writer.write("\t\t\t\t<start>" + p.getStart() + "</start>\r\n");
        this.writer.write("\t\t\t\t<end>" + p.getEnd() + "</end>\r\n");
        if (p.getText() != null) {
          this.writer.write("\t\t\t\t<content xml:space=\"preserve\">" + p.getText());
   ...
   * @throws IOException if an error occurs while writing the output
   */
  public void writeRevision(final Task<Revision> task) throws IOException {
    ...
    while (revIt.hasNext()) {
      ...
      this.writer.write("\t\t" + WikipediaXMLKeys.KEY_START_CONTRIBUTOR.getKeyword());
      if (rev.contributorIsRegistered()) {
        this.writer.write("\t\t" + WikipediaXMLKeys.KEY_START_USERNAME.getKeyword());
        ...
        this.writer.write("\t\t" + WikipediaXMLKeys.KEY_START_IP.getKeyword());
        this.writer.write(rev.getContributorName());
      ...
      this.writer.write("\t\t" + WikipediaXMLKeys.KEY_START_COMMENT.getKeyword());
    ...
    if (task.getTaskType() == TaskTypes.TASK_PARTIAL_LAST || task.getTaskType() == TaskTypes.TASK_FULL) {
