...
public class Link extends ParsedPageObject{
	...
	public enum type {EXTERNAL, INTERNAL, AUDIO, VIDEO, IMAGE};
	...
	public String getContext(int wordsLeft, int wordsRight){
		...
		while( posLeft != 0 && wordsLeft > 0 ){
			while( temp > 0 && text.charAt( temp ) < 48  ) temp--;
			while( temp > 0 && text.charAt( temp ) >= 48 ) temp--;
		...
		while( posRight != text.length() && wordsRight > 0 ){
			while( temp < text.length() && text.charAt( temp ) < 48 )temp++;
			while( temp < text.length() && text.charAt( temp ) >= 48 )temp++;
			posRight = temp;
			wordsRight--;
		}
	...
	public String toString(){
		...
		for( String s: parameters ) result.append("\nLI_PARAMETER: \""+ s +"\"" );
