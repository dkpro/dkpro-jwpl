...
public class DumpVersionFastUtilIntKey
    extends AbstractDumpVersion
{
    ...
    /**
     * after revision parsing the map will be erased and the keys sorted in the array list
     */
    ...
    public Integer categoryIdByTitle(String title)
    {
        ...
        return cNamePageIdMap.containsKey(hash) ? cNamePageIdMap.get(hash) : null;
    }
    ...
    {
        final int hash = title.hashCode();
        return pNamePageIdMap.containsKey(hash) ? pNamePageIdMap.get(hash) : null;
    }
    ...
    public void processRevisionRow(RevisionParser revisionParser)
    {
        ...
        if (rev_timestamp < timestamp) {
            ...
            if (pageIdRevMap.containsKey(rev_page)) {
                processRevisionRowContainsKey(revisionParser, rev_page, rev_timestamp);
            }
            else {
                processRevisionRowMissKey(revisionParser, rev_page, rev_timestamp);
            }
        }

    }

    private void processRevisionRowContainsKey(RevisionParser revisionParser, int rev_page,
            int rev_timestamp)
    {
    ...
    private void processRevisionRowMissKey(RevisionParser revisionParser, int rev_page,
            int rev_timestamp)
    {
