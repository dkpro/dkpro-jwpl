...
public class Revision
{
    ...
    /**
     * Calendar.getInstance().set(2000,0,1) out relative time zero to saving memory
     */
    ...
    {
        Long lowResolutionDate = (date - TIME_ZERO) / MS_IN_SEC;
        return lowResolutionDate.intValue();
    }

    /**
     * Extract time, that was compressed with {@link Revision#compressTime(long)}
     *
    ...
    {
        return (long) compressedDate * MS_IN_SEC + TIME_ZERO;
    }

    /**
     * Merge two unsigned integer values (text id and time stamp) to one long value (revision) to
     * use GNU Trove container.
     */
