 ...
 * added or removed (depending on the mode).
 */
    ...
    implements Serializable
{
    ...
    /**
     * Returns the text "around the given template" and returns the corresponding text in the other
     * pair part of the RevisionPair.
     * <p>
     * Currently, this is done section-based. On TextPairPart contains a section with a template and
     * the other contains the corresponding section after the template has been deleted (in
     * deleteTemplate mode) or before it has been added (in addTemplate mode).
     * <p>
     ...
     * changed, no match will be found.
     *
     * @param markTemplates
     *            sets whether to add an inline marker for the template
    ...
    public List<TextPair> getInlineTextPairs(boolean markTemplates)
    {
        ...
        try {
            ...
            for (ExtractedSection tplSect : revPairType == RevisionPairType.deleteTemplate
                    ? beforeSections
                    : afterSections) {
                ...
                if (containsIgnoreCase(tplSect.getTemplates(), template)) {
                    ...
                    for (ExtractedSection nonTplSect : revPairType == RevisionPairType.deleteTemplate
                            ? afterSections
                            : beforeSections) {
                        // TODO how do we match the sections?
                        // currently only by title - we could do fuzzy matching
                        // of the section body
                        if (tplSect.getTitle() != null && nonTplSect.getTitle() != null
                                && tplSect.getTitle().equalsIgnoreCase(nonTplSect.getTitle())) {
