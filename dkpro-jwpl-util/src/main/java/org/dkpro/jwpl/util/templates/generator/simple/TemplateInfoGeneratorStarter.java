...
public class TemplateInfoGeneratorStarter
{
    ...
    /**
     * Starts index generation using the database credentials in the properties file specified in
     * args[0].<br>
        ...
        if (args == null || args.length != 1) {
            ...
            try {
                ...
                if (outfile.isDirectory()) {
                    try {
                        output = outfile.getCanonicalPath() + File.separatorChar
                                + "templateInfo.sql";
                    }
                    catch (IOException e) {
                ...
                String active_for_pages = props.getProperty(FILTERING_ACTIVE_FOR_PAGES);
                String active_for_revisions = props.getProperty(FILTERING_ACTIVE_FOR_REVISIONS);
                String useRevisionIterator = props.getProperty(USE_REVISION_ITERATOR);
                ...
                if (useRevisionIterator.equals("true") || useRevisionIterator == null
                        || useRevisionIterator.equals("")) {
                    mode.useRevisionIterator = true;
                }

                TemplateFilter pageFilter = new TemplateFilter(
                        createSetFromProperty(props.getProperty(PAGES_WHITE_LIST)),
                        createSetFromProperty(props.getProperty(PAGES_WHITE_PREFIX_LIST)),
                        createSetFromProperty(props.getProperty(PAGES_BLACK_LIST)),
                        createSetFromProperty(props.getProperty(PAGES_BLACK_PREFIX_LIST)));

                TemplateFilter revisionFilter = new TemplateFilter(
                        createSetFromProperty(props.getProperty(REVISIONS_WHITE_LIST)),
                        createSetFromProperty(props.getProperty(REVISIONS_WHITE_PREFIX_LIST)),
                        createSetFromProperty(props.getProperty(REVISIONS_BLACK_LIST)),
                        createSetFromProperty(props.getProperty(REVISIONS_BLACK_PREFIX_LIST)));

                WikipediaTemplateInfoGenerator generator = new WikipediaTemplateInfoGenerator(
                        config, pageBuffer, charset, output, maxAllowedPackets, pageFilter,
                        revisionFilter, mode);
     ...
     * @return Properties the properties object containing the configuration data
     */
    private static Properties load(String configFilePath)
    {
        Properties props = new Properties();
        File configFile = new File(configFilePath);
        try (BufferedInputStream fis = new BufferedInputStream(new FileInputStream(configFile))) {
            props.load(fis);
        }
        catch (IOException e) {
    ...
    /**
     * Parses property string into a Set.
     *
     * @param property
     *            string to parse
     * @return
     */
    public static Set<String> createSetFromProperty(String property)
    {
