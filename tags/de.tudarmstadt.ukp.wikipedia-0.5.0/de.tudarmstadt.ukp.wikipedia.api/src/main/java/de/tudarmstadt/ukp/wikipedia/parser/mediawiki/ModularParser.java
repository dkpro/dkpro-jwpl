...
public class ModularParser implements MediaWikiParser, MediaWikiContentElementParser{
	 ...
	 * Look at MediaWikiParserFactory for a description...
	 */
	public void setLanguageIdentifers( List<String> languageIdentifers){ this.languageIdentifers = listToLowerCase( languageIdentifers ); }
	 ...
	 * Look at MediaWikiParserFactory for a description...
	 */
	public void setCategoryIdentifers( List<String> categoryIdentifers){ this.categoryIdentifers = listToLowerCase( categoryIdentifers ); }
	 ...
	 * Look at MediaWikiParserFactory for a description...
	 */
	public void setImageIdentifers( List<String> imageIdentifers){ this.imageIdentifers = listToLowerCase( imageIdentifers ); }

	/**
	 * Look at MediaWikiParserFactory for a description...
	 */
	public MediaWikiTemplateParser getTemplateParser(){ return templateParser; }

	/**
	 * Look at MediaWikiParserFactory for a description...
	 */
	public void setTemplateParser( MediaWikiTemplateParser templateParser ){ this.templateParser = templateParser; }

	/**
	 * Look at MediaWikiParserFactory for a description...
	 */
	public boolean showImageText(){ return showImageText; }
	 ...
	 * Look at MediaWikiParserFactory for a description...
	 */
	public boolean deleteTags() { return deleteTags; }
	 ...
	 * Look at MediaWikiParserFactory for a description...
	 */
	public boolean showMathTagContent() { return showMathTagContent; }
	 ...
	 * Look at MediaWikiParserFactory for a description...
	 */
	public boolean calculateSrcSpans(){ return calculateSrcSpans; }
	...
	private boolean runConfig(){
		if( lineSeparator == null ){ logger.error( "Set lineSeparator" ); return false; }
		if( categoryIdentifers == null ){ logger.error( "Set categoryIdentifers" ); return false; }
		if( languageIdentifers == null ){ logger.error( "Set languageIdentifers" ); return false; }
		if( imageIdentifers == null ){ logger.error( "Set imageIdentifers" ); return false; }
		if( templateParser == null ){ logger.error( "Set templateParser" ); return false; }
		return true;
	}
	...
	public ParsedPage parse(String src){
		...
		// check if the is something to parse. somtimes there is an empty string due to an error of other clases...
		if(src == null || src.length()==0) {
			return null;
		}

		// creates a new span manager with the given source, appending a newline to avoid errors.
		SpanManager sm = new SpanManager( src.replace('\t', ' ') +lineSeparator);
		if( calculateSrcSpans ) {
			sm.enableSrcPosCalculation();
		}

		// Creating a new ParsePage, which will be filled with information in the parseing process.
		...
		// Removing the Content which should not parsed but integrated later in the resulting text
		...
		// Parseing the Templates (the Span List will be added to the managed lists by the function)
		parseTemplates( sm, cepp.templateSpans, cepp.templates, ppResult );

		// Parsing all other Tags
		parseTags( sm, cepp.tagSpans );

		// Converting &lt;gallery>s to normal Images, this is not beautiful, but a simple solution..
		...
		// Creating a list of Line Spans to work with lines in the following functions
		LinkedList<Span> lineSpans = new LinkedList<Span>();
		getLineSpans( sm, lineSpans );

		// Removing the Category Links from the Links list, and crating an ContentElement for these links...
		ppResult.setCategoryElement( getSpecialLinks(sm, cepp.linkSpans, cepp.links, " - ", categoryIdentifers ) );

		// Removing the Language Links from the Links list, and crating an ContentElement for these links...
		ppResult.setLanguagesElement( getSpecialLinks(sm, cepp.linkSpans, cepp.links, " - ", languageIdentifers ) );

        // Parsing and Setting the Sections... the main work is done in parse sections!
		ppResult.setSections(
				EmptyStructureRemover.eliminateEmptyStructures(
						parseSections(sm, cepp, lineSpans ) ) );
	...
	private void deleteComments( SpanManager sm ){
		...
		while( (start = sm.indexOf("<!--", start) )!= -1 ){
			 ...
			 try{
				 if( 	lineSeparator.equals( sm.substring( start-lineSeparator.length(), start) ) &&
						lineSeparator.equals( sm.substring( end, end+lineSeparator.length() ))			) {
	...
	private SectionContainer parseSections(SpanManager sm, ContentElementParsingParameters cepp, LinkedList<Span> lineSpans){
		...
		// Identify the Line Type and call the necessary Function for the further handling...
		while( !lineSpans.isEmpty() ){
			...
			case SECTION:
				...
				sc = new SectionContent( parseContentElement(sm, cepp, new Span(s.getStart()+level, s.getEnd()-level).trim( sm ) ), level );
				lineSpans.removeFirst();
	...
	private lineType getLineType( SpanManager sm, Span lineSpan ){
		...
		case '=':	if(	lineSpan.length()>2 && sm.charAt(lineSpan.getEnd()-1)=='=') {
		...
		case '*':	return lineType.NESTEDLIST;

		case '#':	return lineType.NESTEDLIST_NR;

		case ';':	return lineType.DEFINITIONLIST;

		case ':':	if( lineSpan.length()>1 ) {
		...
		case ' ':	int nonWSPos = lineSpan.nonWSCharPos(sm);
					switch( lineSpan.charAt(nonWSPos, sm) ){
					case Span.ERRORCHAR:	return lineType.EMPTYLINE;
					case '{':				if( lineSpan.charAt(nonWSPos+1, sm)=='|' ) {
						return lineType.TABLE;
					}
					default:				return lineType.PARAGRAPH_BOXED;
					}

		case Span.ERRORCHAR: return lineType.EMPTYLINE;

		default:	return lineType.PARAGRAPH;
		}
	}
	...
	/**
	 * Takes a list of SectionContent and returns a SectionContainer with the given
	 * SectionContent s in the right structure.
	 */
	...
	private void parseTemplates(SpanManager sm, List<Span> resolvedTemplateSpans, List<ResolvedTemplate> resolvedTemplates, ParsedPage pp ){
		...
		while( !templateOpenTags.empty() ){
			...
			Template t = new Template(
					ts,
					encodeWikistyle( sm.substring(templateOpenTag+2, templateNameEnd).trim() ),
					templateOptions
					);

			if( calculateSrcSpans ) {
				t.setSrcSpan( new SrcSpan(
						sm.getSrcPos( templateOpenTag ),
						sm.getSrcPos( templateCloseTag+2 )
					));
			}
	...
	private void convertGalleriesToImages(SpanManager sm, List<Span> tagSpans){
		// Quick Hack, not very efficent, should be improved, wont work with calculateSrcSpans == true !
		...
		for( int i=0; i<tagSpans.size()-1; i++ ){
			...
			if( startsWithIgnoreCase(openText, "GALLERY" )){
				...
				if( startsWithIgnoreCase( getTagText( sm, tagSpans.get(i+1) ), "/GALLERY" ) ){
					...
					//images
					for( String s: tokenize( sm, startSpan.getEnd(), endSpan.getStart(), lineSeparator ) ) {
	...
	private Table buildTable(SpanManager sm, ContentElementParsingParameters cepp, LinkedList<Span> lineSpans){
		...
		while( !lineSpans.isEmpty() ){
			...
			if( subTables==0 && (c0=='!' || c0== '|') ){
				...
				if( c1 == '-' ){
					...
					result.setTitleElement( parseContentElement(sm, cepp, new Span( s.getStart()+pos+2, s.getEnd() ).trim(sm) ));
					continue;
				}
				else{
					int multipleCols;
		...
		if( tableDataSpans.size()!=0 ){
			...
			if( calculateSrcSpans ) {
				ei = new SrcSpan(
						sm.getSrcPos( tableDataSpans.getFirst().getStart()-1 )+1,
						-1
					);
			}
	...
	private DefinitionList buildDefinitionList(SpanManager sm, ContentElementParsingParameters cepp, LinkedList<Span> lineSpans ){
		...
		if( calculateSrcSpans ) {
			result.setSrcSpan( new SrcSpan( sm.getSrcPos( s.getStart() ), content.get( content.size()-1 ).getSrcSpan().getEnd() ));
		}
	...
	private List<String> tokenize( SpanManager sm, int start, int end, String delim ){
		...
		if( start>end ){
			logger.debug( "tokenize(" +start +", "+ end+ ") doesn't make sense");
			return result;
		}
	...
	private void parseExternalLinks( SpanManager sm, Span s, String protocol,List<Span> managedList, List<Link> links, Content home_cc ){
		...
		while( (extLinkTargetStart = sm.indexOf(protocol, extLinkSpan.getEnd(), s.getEnd() )) != -1 ){
			...
			while(
					( lineSeparator+" ]" ).indexOf( sm.charAt( extLinkTargetEnd ) )==-1
				) {
				extLinkTargetEnd++;
			}
			...
			if( extLinkOpenTag >= s.getStart() && sm.charAt(extLinkOpenTag)=='['){
				...
				if( extLinkCloseTag!=-1 ){
					...
					// nicht wie bei "normalen" links durhc | getrennt sondenr durhc leerzeichen !!! schei�e !!!
	...
	private void parseImagesAndInternalLinks(SpanManager sm, List<Span> linkSpans, List<Link> links){
		...
		while( !linkOpenTags.empty() ){
			...
			if( namespace != null && imageIdentifers.indexOf( namespace )!= -1){
				if( linkOptionTag != -1 ){
					int temp;
					...
					//maybe there is an external link at the end of the image description...
	...
	/**
	 * Searches the Range given by the Span s for the double occurence of "quotation" and
	 * puts the results in the List quotedSpans. The Quotation tags will be deleted.
	 *
	...
	/**
	 * Building a ContentElement from a single line. But the result is given, so e.g. a NestedListElement can be filled with information...
	 */
	...
	private ContentElement parseContentElement( SpanManager sm, ContentElementParsingParameters cepp, LinkedList<Span> lineSpans, ContentElement result ){
		...
		//set the SrcSpan
		if( calculateSrcSpans ){
			result.setSrcSpan( new SrcSpan(
					sm.getSrcPos( contentElementRange.getStart() ),
		...
		//managed spans must be removed here and not earlier, because every change in the SpanManager affects the Spans!
	...
	private static List<Link> sortLinks( List<Link> links ){
		...
		for( Link l: links ){
			...
			while( pos<result.size() &&
					l.getPos().getStart()
					>
					result.get(pos).getPos().getStart() ) {
				pos++;
			}
			result.add( pos, l );
		}
	...
	private void setFirstParagraph( ParsedPage pp ){
		...
		// the paragraph with the lowest number, must not be the first, maybe it is only an Image...
		for(int i=0; i<nr; i++ ){
			...
			// delete the spans in reverse order, the spans are managed, so there is no need to sort them
			for( int j=delete.size()-1; j>=0; j-- ) {
				ptext.delete( delete.remove(j) );
			}

			// removing line separators if exist, so the result can be trimmed in the next step
			...
			// if the result is not an empty string, we got the number of the first paragraph
