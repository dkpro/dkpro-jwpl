...
public class SingleDumpVersionOriginal implements IDumpVersion {
  ...
  public void initCategoryLinksParsing() throws IOException {
    pageCategories = new TxtFileWriter(outputDir + File.separator + "page_categories.txt");
    categoryPages = new TxtFileWriter(outputDir + File.separator + "category_pages.txt");
    categoryInlinks = new TxtFileWriter(outputDir + File.separator + "category_inlinks.txt");
    categoryOutlinks = new TxtFileWriter(outputDir + File.separator + "category_outlinks.txt");
  }

  @Override
  public void initPageLinksParsing() throws IOException {
    pageInlinks = new TxtFileWriter(outputDir + File.separator + "page_inlinks.txt");
    pageOutlinks = new TxtFileWriter(outputDir + File.separator + "page_outlinks.txt");
  }
  ...
  public void initTextParsing() throws IOException {
    ...
    pageMapLine = new TxtFileWriter(outputDir + File.separator + "PageMapLine.txt");
    pageRedirects = new TxtFileWriter(outputDir + File.separator + "page_redirects.txt");
  ...
  public void processPageLinksRow(PagelinksParser plParser) throws IOException {
    ...
    if (skipPage && !pPageIdNameMap.containsKey(pl_from) || !pNamePageIdMap.containsKey(pl_to)) {
      return;
    }
  ...
  public void processTextRow(TextParser textParser) throws IOException {
    ...
    if (pPageIdNameMap.containsKey(page_id)) {// pages
      page.addRow(page_id, page_id, pPageIdNameMap.get(page_id),
              textParser.getOldText(), formatBoolean(disambiguations.contains(page_id)));
  ...
  public void writeMetaData() throws IOException {
    try(TxtFileWriter metaData = new TxtFileWriter(outputDir + File.separator + "MetaData.txt")) {
